...
├── mock_hal_adc.c            # 【CMock 桩】HAL ADC 桩实现（CMock 生成，已检入）
├── mock_hal_adc.h            # 【CMock 桩】HAL ADC 桩头文件（CMock 生成，已检入）
│
...
│   └── unity_internals.h
│
└── cmock/                    # 【CMock】源码（已 vendor）
    ├── lib/                  # CMock Ruby 生成器库（cmock.rb + 各插件）
    ├── config/               # CMock Ruby 环境配置（production_environment.rb）
    └── src/                  # CMock C 运行时（cmock.c/h，编译桩时需链接）
```
...
| Ruby | 运行 `gen_mocks.rb` 重新生成 CMock 桩 | 桩文件已由 CMock 生成并检入，可直接编译运行 |
...
`mock_hal_adc.c/.h` 由 CMock 从 `hal_adc.h` 自动生成，并已检入仓库。这样即使在**没有 Ruby 的环境**下也能直接编译运行。

- CMock 生成的桩使用**动态内存分配**（malloc/realloc），预设返回值数量可动态增长
- 桩文件头部标注 `AUTOGENERATED FILE. DO NOT EDIT.`，请勿手动修改
- 编译时需链接 `cmock/src/cmock.c`（CMock 运行时，提供桩函数依赖的内存管理）

**有 Ruby 环境时**，可重新生成桩（会覆盖当前版本）：
...
### Q3：桩文件是 CMock 生成的，为什么要检入仓库？

**A**：为了让工程在**没有 Ruby 的环境**下也能直接编译运行。CMock 生成桩需要 Ruby 环境，但编译和运行测试只需要 gcc。

- 仓库中已检入 CMock 生成的 `mock_hal_adc.c/.h`，开箱即用
- 有 Ruby 环境时运行 `ruby gen_mocks.rb` 可重新生成（会覆盖当前版本）
- CI 流水线中会重新生成桩，确保 `hal_adc.h` → 桩的自动生成流程始终可用

CMock 生成的桩使用动态内存分配（malloc/realloc），预设返回值数量可动态增长，适合单元测试场景。编译时需链接 `cmock/src/cmock.c`（CMock 运行时）。
...
> 没有 Ruby 环境时，可跳过步骤 4（使用仓库中已检入的 CMock 桩），其余步骤均可正常执行。
